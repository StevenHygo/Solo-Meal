...
- Outbox 待处理、投递中、失败、已完成状态，带 `SKIP LOCKED` 租约回收、指数退避和人工重试的独立 Webhook Worker。
- 运营台审计/投递工作区，以及餐厅、POI 候选、复核任务和审计日志的固定字段 CSV 导出。
- 审计、投递与导出管理 API，fixture/PostgreSQL 一致实现和队列并发回归测试。
...
- 获授权真实 Provider Adapter、覆盖区域开关和证据即将过期专用列表。
