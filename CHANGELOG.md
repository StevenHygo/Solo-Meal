...
- 城市与覆盖区域状态开关，保留区域配置状态并以城市暂停状态覆盖公开搜索结果。
- 证据即将过期管理 API 和运营列表，支持时间窗口、覆盖区域与核验属性筛选。
- 排序配置草稿、发布和回滚 API，fixture/PostgreSQL 事务实现，以及活动版本运营控件。
- 排序配置权重总和、重复版本、审计失败回滚和跨版本分页游标回归测试。
...
- 搜索、详情和配置接口统一读取活动排序版本，配置切换后旧分页游标失效。
