...
- 受令牌保护的最小运营台，可认领、释放、完成或驳回复核任务，并扫描过期证据；
...
POI 导入只接受附带授权依据的标准化 JSON，并停留在候选队列；确认“新分店”也不会直接进入搜索。文件格式、状态机和 `coverage-beta-v1` / `coverage-live-v1` 门槛见 [Stage C POI 运维约定](./docs/stage-c-poi-operations.md)。
...
校验覆盖 Web 静态资源和双数据源契约、小程序回归、TypeScript 类型、环境开关、API schema、鉴权和幂等契约、任务转换、坐标转换、营业时间、迁移回滚、排序/分页，以及浏览器、服务端品类和覆盖配置一致性。
...
当前 `1.0.0-beta.1` 已完成阶段 A/B，并完成阶段 C 的服务端纠错、复核任务、审计/Outbox、证据过期扫描、Provider-neutral POI 候选导入/去重和版本化覆盖门槛报告；fixture 模式、故障降级和浏览器闭环已验证。由于本机没有 Docker/`psql`，真实 PostGIS 的迁移、种子、空间索引与回滚执行仍需在可用环境演练。阶段 C 尚未接入获授权的真实 Provider 数据，也未完成新分店核心字段/证据录入、二次审核、发布/撤回和任何新增区域的 beta 门槛；当前运营台也不替代生产所需的 MFA/RBAC。达到这些门槛并完成备份恢复、隐私、地图条款和内容授权评审前，不应扩大公开覆盖或把 API 设为默认数据源。
