...
当前 `1.0.0-beta.1` 已完成阶段 A/B，并完成阶段 C 的服务端纠错、复核任务、审计/Outbox、证据过期扫描和最小运营台；fixture 模式、故障降级和浏览器闭环已验证。由于本机没有 Docker/`psql`，真实 PostGIS 的迁移、种子、空间索引与回滚执行仍需在可用环境演练。阶段 C 尚未完成真实 POI 候选导入、去重、字段核验、审核发布和新增区域 beta 门槛；当前运营台也不替代生产所需的 MFA/RBAC。达到这些门槛并完成备份恢复、隐私、地图条款和内容授权评审前，不应扩大公开覆盖或把 API 设为默认数据源。
